...
The present package offers two options for the `weightedEL()` function: `chull.fail = "taylor"` that extrapolates its branches halfway after the penultimate observation at each side, or `chull.fail = "wald"` that replaces the SELR function halfway between the outermost observations with a (discontinuous) Wald approximation.

```{r}
x <- c(-5, -3:3, 7)
grid.full <- seq(-5.5, 7.5, 0.1)
selr0 <- sapply(grid.full, function(m) -2*weightedEL(x, mu = m, chull.fail = "none")$logelr)
selr <- sapply(grid.full, function(m) -2*weightedEL(x, mu = m, chull.fail = "taylor")$logelr)
plot(grid.full, selr, ylim = c(0, 60), xlim = c(-5.5, 7.5),
     col = 1 + (grid.full < -4 | grid.full > 5), bty = "n",
     main = "-2 * weighted log-EL ratio")
lines(grid.full, selr0, lty = 1)
lines(grid.full, (grid.full - mean(x))^2 / (var(grid.full) / length(x)), lty = 2)
rug(x, lwd = 2)
```

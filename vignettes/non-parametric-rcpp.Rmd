...
date: "Created: 2023-12-05, last modified: 2024-06-10, compiled: `r Sys.Date()`"
...
Kernel weights define a local neighborhood for a point using the value of the kernel function:
\[
...
microbenchmark(
  wCPP <- kernelWeights(X, xout = xg, bw = b),
  wR   <- kernelWeightsR(X, xout = xg, bw = b),
  times = 20, unit = "ms", check = "equivalent"
)

microbenchmark(
  fCPP <- kernelDensity(X, xout = xg, bw = b),
  fR   <- kernelDensityR(X, xout = xg, bw = b),
  times = 20, unit = "ms", check = "equivalent"
)

microbenchmark(
  mCPP <- kernelSmooth(X, Y, xout = xg, bw = b),
  mR   <- kernelSmoothR(X, Y, xout = xg, bw = b),
  times = 20, unit = "ms", check = "equivalent"
)
```
...
These examples showcase the speed improvements and comparable accuracy of the Rcpp implementations over pure R code, making the `smoothemplik` a powerful tool for non-parametric statistical analysis.

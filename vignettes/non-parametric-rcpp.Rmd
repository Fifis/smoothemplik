...
date: "Updated: 2023-12-05, compiled: `r Sys.Date()`"
...
1. *** TODO: Memory-saving of kernelWeights
...
microbenchmark(
  wCPP <- kernelWeights(X, xout = xg, bw = bw),
  wR   <- kernelWeightsR(X, xout = xg, bw = bw),
  times = 20, unit = "ms", check = "equivalent"
)

microbenchmark(
  fCPP <- kernelDensity(X, xout = xg, bw = bw),
  fR   <- kernelDensityR(X, xout = xg, bw = bw),
  times = 20, unit = "ms", check = "equivalent"
)

microbenchmark(
  mCPP <- kernelSmooth(X, Y, xout = xg, bw = bw),
  mR   <- kernelSmoothR(X, Y, xout = xg, bw = bw),
  times = 20, unit = "ms", check = "equivalent"
)
```
...
We can visualise the result and see that the discrepancies are tiny
```{r}
...
```{r message=FALSE}
par(mfrow = c(1, 1))
fCPPunif <- kernelDensity(X, xout = xg, bw = bw * sqrt(3), kernel = "uniform")
fCPPtrng <- kernelDensity(X, xout = xg, bw = bw * sqrt(3), kernel = "triangular")
fCPPepan <- kernelDensity(X, xout = xg, bw = bw * sqrt(5), kernel = "epanechnikov")
...
timings <- sapply(ns, function(n) {
  ...
  a <- summary(microbenchmark(
    ...
    times = 3, unit = "ms", check = "equivalent", control = list(warmup = 1)
  ))
...
microbenchmark(
  fCPP <- kernelDensity(X, xout = xg, bw = bw),
  fR <- kernelDensityR(X, xout = xg, bw = bw),
times = 5, unit = "ms", check = "equivalent")
all.equal(fR, fCPP, tolerance = 1e-16)
```
...
Finally, we can visualise the kernel regression estimator with various kernels and degrees, and robustness operations (described in the original Cleveland 1979 LOESS paper). TODO: add reference.

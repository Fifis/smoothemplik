...
# ctracelr returns: a matrix with one row at each mean from mu0 to mu1
...
cemplik = function( z,  # matrix with one data vector per row, a column vector is ok when d=1
                   ct,  # vector of counts
...
init0 = mllog( rep(1,n), eps=eps, M=M, der=2 ) # i.e. fn and derivatives at lam = 0
...
logelr = sum( ct*mllog(1+z%*%lam,eps=eps,M=M,der=0) )
...
ctracelr = function( z, ct, mu0, mu1, N, verbose=FALSE, ... ){
